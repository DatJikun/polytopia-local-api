...
def _finalize_cities(cities: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # Real enemies are already frost-capped in _cap_vengir; do not slice them
    # behind an own-city quota. Own still caps so Bardur fragments cannot flood.
...
def _cap_vengir(cities: list[dict[str, Any]], unnamed_limit: int = 2) -> list[dict[str, Any]]:
    ...
    # Enemies first so a later combined cap cannot drop Disrof behind Bardur.
