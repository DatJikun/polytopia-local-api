...
def test_twelve_own_cities_do_not_drop_enemy():
    """Live T45: 12 Bardur sticky filled the old [:12] cap → cities_enemy [].

    Frost FPs stay capped at 2; Disrof-class hits are not in that quota.
    """
    from polytopia_api.entities import session_cities
...
def test_distinct_vengir_do_not_merge_at_80px():
    """Disrof vs Rzgórst on adjacent hex rows used to collapse at max(dist, 80)."""
    from polytopia_api.entities import _finalize_cities

    a = _enemy_city("c10_4", [10, 4], 400, 300, plate_y=330, w=40)
    b = _enemy_city("c11_6", [11, 6], 430, 370, plate_y=400, w=40, evidence=["magenta_roof", "frost_plate"])
    out = _finalize_cities([a, b])
    ids = {c["id"] for c in out}
    assert ids == {"c10_4", "c11_6"}, out
...
if __name__ == "__main__":
    ...
    test_twelve_own_cities_do_not_drop_enemy()
