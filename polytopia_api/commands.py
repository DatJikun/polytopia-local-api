...
def _is_city_hit(hit: dict[str, Any] | None) -> bool:
    if not hit:
        return False
    return bool(hit.get("kind") == "city" or hit.get("plate_y") is not None or hit.get("tile") is not None)
...
def _move_to(
    ...
    if is_city:
        ...
        if mark is None and (not marks) and (not enemy_on) and reach.get("ok"):
