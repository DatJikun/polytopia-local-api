...
TOOLS: list[dict[str, Any]] = [
    ...
    {
        "name": "local_observe",
        "description": "Screenshot observe: HUD, stable units/cities (Bardur stay cities_own even 1 hex from Vengir and never flip own→enemy; cities_own matches Game Stats / screen — frost-plate + gold star/lamps on a longhouse below the HUD, including sparse live wood (warm≈260 dens≈0.08 ww≳36 wh≳60) even when gold_n dips below 24, the plate splits (n/w below 900/70), or recruit/move leaks gold onto a fragment; a ww≳36 longhouse is never grey_stone/wine-salvaged as enemy (live Bufla hashed as c12_16 ww≈47); close-plate dupes (~35px Orkork c15_24/c16_24) merge and must not steal the longhouse city_id; gold-star fragments ~72px (c15_12 vs Orkork) merge when one hit is a fragment; 1-hex neighbors ~80px stay two; a missed-frame Orkork/c16_24 stays sticky (cities_own does not flicker 2→1); mid-map unit/mountain phantoms, frost/wood false plates (c15_15 / c15_12 / c15_24 — gold star without longhouse wood, including gold_n≥24 leak after /recruit+/move-to) and tall-empty frost (c17_14 dens≈0.02) drop from own; tall-dense-low-gold grey stone (narrow slat c12_16 dens≈0.28 ww≈32 gold_n≈5 → cities_enemy) and HUD-fog (c18_1 y≈45) drop from own and do not sticky-grow 2→4; visible Vengir stay cities_enemy even when cities_own is full / 12+ Bardur; distinct Disrof/Rzgórst/grey-stone neighbors do not merge at ~80px kissing banners; dark frost+star wine plates (c14_23) stay cities_enemy even when sampled/col tribes are both Bardur, veins are light wine-grey (g≈74–86) or wine_n==0 on a city-sized plate (Game Stats Vengir 5); Disrof stays cities_enemy while a friendly unit is on the port/mountain next to it; those units keep sticky ids), fog_edge. Villages off unless POLYTOPIA_VILLAGES=1. Includes turn_diff vs last End Turn snapshot.",
        "inputSchema": {"type": "object", "properties": {}},
    },
    {
        "name": "local_hud",
