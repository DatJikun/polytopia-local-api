...
TOOLS: list[dict[str, Any]] = [
    ...
    {
        "name": "local_observe",
        "description": "Screenshot observe: HUD, stable units/cities (Bardur stay cities_own even 1 hex from Vengir and never flip own→enemy; cities_own matches Game Stats / screen — frost-plate + gold star/lamps on a longhouse below the HUD, including sparse live wood (warm≈260 dens≈0.08 ww≳36 wh≳60) even when gold-star hole splits plate n/w below 900/70, and when gold_n dips below 24 on a city-sized plate; close-plate dupes (~35px Orkork c15_24/c16_24) merge and must not steal the longhouse city_id; gold-star fragments ~72px merge when one hit is a fragment; 1-hex neighbors ~80px stay two; a missed-frame Orkork/c16_24 stays sticky (cities_own does not flicker 2→1); mid-map unit/mountain phantoms, frost/wood false plates (c15_15 / c15_12 / c15_24 — gold star without a city-sized longhouse, including gold_n≥24 leak after /recruit+/move-to even when ww inflates ≳36 / gold≈40 warm≈180) and tall-empty frost (c17_14 dens≈0.02) drop from own; tall-dense-low-gold grey stone (c12_16 dens≈0.28 gold_n≈5 ww≈32–47 → cities_enemy, the 5th Vengir) and HUD-fog (c18_1 y≈45) drop from own and do not sticky-grow 2→3 after recruit/move; visible Vengir stay cities_enemy even when cities_own is full / 12+ Bardur; distinct Disrof/Rzgórst/grey-stone neighbors do not merge at ~80px kissing banners; dark frost+star wine plates (c14_23) stay cities_enemy even when sampled/col tribes are both Bardur, veins are light wine-grey (g≈74–86), ash-grey (96,92,90), or wine_n==0 on a near-city plate (Game Stats Vengir 5); gold-lamp Orkork never flips own→enemy after /recruit (wine-shadow roof_n≈8 is not Disrof); leak fragments c15_14/c16_12/c8_23 do not bloat cities_enemy; Disrof stays cities_enemy while a friendly unit is on the port/mountain next to it; those units keep sticky ids; GET /observe stays cities_own==2 and cities_enemy==5 across clean frames and after /recruit+/move-to), fog_edge. Villages off unless POLYTOPIA_VILLAGES=1. Includes turn_diff vs last End Turn snapshot.",
        "inputSchema": {"type": "object", "properties": {}},
    },
    {
        "name": "local_hud",
    ...
    {
        "name": "local_move_to",
        "description": "Walk onto a city tile (city_id or x,y). If garrisoned, returns city_occupied with next=/attack. Else adjacent (~1 hex) clicks the city-foot / blue ring even when clustered move_marks are empty. After the click, waits for the walk, re-selects if still adjacent, and verifies the unit tile. Sticky city_id stays resolvable all turn. stood_on_city true only when the unit is ON the hex.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "x": {"type": "integer"},
