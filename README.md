...
- `POST /capture` — gate ON tile (~0.8 hex, nie adjacent ~1.0) **albo** panel Capture + unit przy mieście (HP na nameplate / sticky port ~1.0). `unit.capture` / blob w UNIT_CROP; bez standing `not_standing_on_city`. OCR Capture bez bloba klika panel, nie mapowe `do_it`. `captured` gdy `cities_enemy` straci ten `city_id` albo owner→own.
