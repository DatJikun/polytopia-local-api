...
3. `GET /observe` — `units` / `cities_own` / `cities_enemy` ze **stabilnym `city_id` = `c{gx}_{gy}`** (hash kafelka, nie indeks listy) i `cities_enemy[].tile: [gx,gy]`. **`city_id` i `unit_id` zostają na całą turę** — attack/move-to/capture resolvują `c22_17` / `u2` nawet gdy kolejny observe zgubi frost plate albo pasek HP (indeks sticky + `seen:false`). `cities_own` to Bardur (`owner:own`); gold star na frost plate **nie** robi z nich Vengir. Enemy Vengir = **magenta roof albo gold lamps na budynku + frost plate** (`evidence`, `confidence` ≥0.78); gołe frost fragmenty i ghost `Arch` odpadają (max 2 unnamed vengir) i **nie kumulują się** między observe (2→4→7). Jasne oumaji-ghosty bez purple/gold-on-dark odpadają. `villages` zwykle `[]`; `fog_edge`; HUD `turn`/`stars`/`score` z OCR + cropów cyfr. `hud.turn_trusted` jest **false** gdy OCR tury jest poniżej last labeled/clocked (`turn_floor` z `latest.json`, najwyższego `T{n}.json`, albo `POLYTOPIA_TURN=`); `turn` wtedy `null`, `turn_ocr` zostaje (np. 2 przy T26). `stale` / `missing` gdy cache. `turn_diff.alerts`.
4. W-path (Disrof): `POST /attack {"from_id":"uX","city_id":"cY"}` — czerwony mark przy **garrison unit xy** (albo czerwony tint pod kafelkiem), HP z paska unita na kafelku (nie `n`/`w` plate), `hp_dropped` albo jasne `no_actions` / `no_mark` **szybko** (nie timeout 7.5s). `POST /move-to {"from_id":"uZ","city_id":"cY"}` — niebieski mark najbliższy **środka tile** (nie plate); gdy unit jest w zasięgu chodu w tej turze, klika `tile_xy` nawet bez sklastrowanego marka; `stood_on_city`. Bez wejścia na miasto → `no_mark_on_city_tile` + `path_reason` (`out_of_move_range` / `city_occupied` / `no_move_marks`) + `suggested_tile_xy`. `POST /capture {"city_id":"cY"}` tylko gdy unit ON (<0.4 hex) + blob w UNIT_CROP; inaczej `not_standing_on_city` bez kliku; `captured` gdy enemy straci `city_id`. `POST /recruit {"city_id":"c0"}` — nameplate → TRAIN blob.
...
POST /move-to  {"from_id":"uZ","city_id":"cY"} → stood_on_city true when marks exist (city_id wciąż resolvuje)
...
- `cities` / `cities_own` / `cities_enemy` — Moonrise **frosted** nameplate (szary ~180, nie 215-white) + kolor plemienia. `id` = `c{gx}_{gy}`, pole `tile`. **Sticky na turę** (lookup po `city_id` działa po ataku nawet gdy frame zgubi miasto). `cities_own` = Bardur mimo złotej gwiazdy na plate. Vengir = fioletowy dach **albo złote lampy na budynku** (`evidence: magenta_roof|gold_lamps`, wyższy `confidence`); gołe frost FPs i OCR-ghost `Arch` odpadają i nie rosną w sesji. Jasny oumaji-looking na gorącej białej płytce odpada. `name` opcjonalne (OCR płytek wyłączone — `/attack` <8s). Lookup po `city_id`.
- `POST /move-to` — niebieskie marki po select `from_id`; cel = środek budynku (~`plate_y` minus ~0.75 hex). W zasięgu chodu klika `tile_xy` (nawet gdy klaster nie złapał blue). `stood_on_city` po re-observe. Negatyw: `no_mark_on_city_tile` + `suggested_tile_xy` + `path_reason`. `ok` zawsze bool (nigdy `null`).
- `POST /attack` — po select tylko lekki observe (panel + marki, bez HUD/OCR miast). Brak czerwonego → szybki `no_mark` / `no_attack_marks`, nie timeout 7.5s.
