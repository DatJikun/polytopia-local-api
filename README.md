...
3. `GET /observe` — `units` / `cities_own` / `cities_enemy` ze **stabilnym `id` i `city_id` = `c{gx}_{gy}`** (hash kafelka, nie indeks listy) i `cities_enemy[].tile: [gx,gy]`. **`city_id` i `unit_id` zostają na całą turę** — attack/move-to/capture resolvują `c22_17` / `u2` nawet gdy kolejny observe zgubi frost plate albo pasek HP (indeks sticky + `seen:false`). Druga komenda w tej samej turze **nie** wraca `need x,y or city_id/to_id` dla znanego `c14_24`. **Disrof zostaje w `cities_enemy`** gdy jest na ekranie — nawet z garnizonem i unitami Bardur na **porcie na wschód** albo górze na południe; nie znika i nie wpada do `cities_own`. **Unit na górze od razu na południe od Disrof** (`tile_xy≈[600,144]`) **albo na porcie na wschód** jest w `units` na pierwszym observe (sticky `id` + `tile`, `near_city_id`) — pasek HP nad śniegiem/wodą nie jest wodą; skóra Bardur na hexie sąsiednim też. Nie tylko wojownicy na dole (~y=462). `cities_own` to Bardur (`owner:own` / `tribe:bardur`); gold star na prawej stronie frost plate **nie** robi z nich Vengir. Enemy Vengir = **magenta roof albo gold lamps na budynku albo złote okna na frost plate** (`evidence`, `confidence` ≥0.78) — frost-plate gold zostaje enemy, **nie** wymyśla `cities_own`. Gołe frost fragmenty, ghost `Arch` i nienazwany Imperius bez frost plate odpadają (max 2 unnamed vengir) i **nie kumulują się** między observe (2→4→7). Jasne oumaji-ghosty bez purple/gold-on-dark odpadają. `villages` zwykle `[]`; `fog_edge`; HUD `turn`/`stars`/`score` z OCR + cropów cyfr. `hud.turn_trusted` jest **false** gdy OCR tury jest poniżej last labeled/clocked (`turn_floor` z `latest.json`, najwyższego `T{n}.json`, albo `POLYTOPIA_TURN=`); `turn` wtedy `null`, `turn_ocr` zostaje (np. 2 przy T26). `stale` / `missing` gdy cache. `turn_diff.alerts`.
4. W-path (Disrof): jeśli `/move-to` zwraca `city_occupied`, **najpierw atakuj garnizon** (`next_call` `/attack` z `attacker_id` lądowym — nie tratwa). `POST /attack {"from_id":"uX","city_id":"cY"}` — czerwony mark przy **stopie kafelka** (nie pasek HP), `hp_dropped` aż `garrison_dead` / `next:move-to`, albo jasne `no_actions` / `no_mark` / `naval_no_land` **szybko**. Potem `POST /move-to` — klika **stopę hexu** (nie dach). Gdy unit jest ~1 hex od miasta i clustered `move_marks` są puste, i tak klika stopę / ring (nie `no_move_marks`). `ok` true **tylko** przy `stood_on_city`. `POST /capture` gdy unit ON + panel Capture; gdy garnizon żyje → `city_occupied`. `POST /recruit {"city_id":"c0"}` — nameplate → TRAIN blob w panelu.
...
POST /attack   {"from_id":"land_unit","city_id":"cY"} → hp_dropped until garrison_dead (or clear no_mark / naval_no_land, <8s)
...
- `cities` / `cities_own` / `cities_enemy` — Moonrise **frosted** nameplate (szary ~180, nie 215-white) + kolor plemienia. `id` **i** `city_id` = `c{gx}_{gy}`, pole `tile`. **Sticky na turę** (lookup po `city_id` działa po ataku nawet gdy frame zgubi miasto; druga komenda nie wraca `need x,y`). Disrof zostaje `cities_enemy` gdy widać miasto (garnizon / Bardur na porcie lub górze nie czyści listy). `cities_own` = Bardur mimo złotej gwiazdy na prawej stronie plate. Vengir = fioletowy dach **albo złote lampy na budynku albo złote okna na frost plate** (`evidence: magenta_roof|gold_lamps`, wyższy `confidence`); frost-plate gold zostaje enemy (nie wymyśla own). Gołe frost FPs i OCR-ghost `Arch` odpadają i nie rosną w sesji. Jasny oumaji-looking na gorącej białej płytce odpada. `name` opcjonalne (OCR płytek wyłączone — `/attack` <8s). Lookup po `city_id`.
- `POST /move-to` — niebieskie marki po select `from_id`; cel = **stopa hexu** (~`plate_y` − 0.45 pitch). Garnizon enemy → `city_occupied` + `next:/attack` (bez kliku w blue). Gdy unit jest ~1 hex od `city_id` i clustered marki są puste, klika stopę (nie `no_move_marks`). `stood_on_city` po re-observe; `ok` true tylko wtedy. Negatyw: `no_mark_on_city_tile` / `out_of_range` / `not_stood_on_city`.
- `POST /attack` — czerwony hex na stopie miasta (nie pasek HP). `hp_dropped`; gdy garnizon znika → `garrison_dead` + `next:/move-to`. Raft → `naval_no_land` + `attacker_id` lądowego sąsiada. Brak czerwonego → szybki `no_mark`.
- `POST /capture` — gate ON tile **albo** panel Capture + unit przy mieście (HP na nameplate / sticky port ~1.0). Garnizon enemy → `city_occupied` (atakuj najpierw). OCR Capture bez bloba klika panel, nie mapowe `do_it`. `captured` gdy `cities_enemy` straci ten `city_id`.
