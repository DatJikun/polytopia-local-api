...
3. `GET /observe` — `units` / `cities_own` / `cities_enemy` ze **stabilnym `city_id` = `c{gx}_{gy}`** (hash kafelka, nie indeks listy) i `cities_enemy[].tile: [gx,gy]`. **`city_id` i `unit_id` zostają na całą turę** — attack/move-to/capture resolvują `c22_17` / `u2` nawet gdy kolejny observe zgubi frost plate albo pasek HP (indeks sticky + `seen:false`). `cities_own` to Bardur (`owner:own`); Vengir FP nie zjada własnych miast. Enemy Vengir = **magenta roof albo gold lamps na ciemnym kamieniu + frost plate** (`evidence`, `confidence` ≥0.78); gołe frost fragmenty odpadają (max 2 unnamed vengir) i **nie kumulują się** między observe (2→4→7). Jasne oumaji-ghosty bez purple/gold-on-dark odpadają. `villages` zwykle `[]`; `fog_edge`; HUD `turn`/`stars`/`score` z OCR + cropów cyfr. `hud.turn_trusted` jest **false** gdy OCR tury jest poniżej last labeled/clocked (`turn_floor` z `latest.json`, najwyższego `T{n}.json`, albo `POLYTOPIA_TURN=`); `turn` wtedy `null`, `turn_ocr` zostaje (np. 2 przy T26). `stale` / `missing` gdy cache. `turn_diff.alerts`.
