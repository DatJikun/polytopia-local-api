...
4. W-path (Disrof): jeśli `/move-to` zwraca `city_occupied`, **najpierw atakuj garnizon** (`next_call` `/attack` z `attacker_id` lądowym — nie tratwa). `POST /attack {"from_id":"uX","city_id":"cY"}` — czerwony mark przy **stopie kafelka** (nie pasek HP), `hp_dropped` aż `garrison_dead` / `next:move-to`, albo jasne `no_actions` / `no_mark` / `naval_no_land` **szybko**. Potem `POST /move-to` — klika **stopę hexu** (nie dach). Gdy unit jest ~1 hex od miasta i clustered `move_marks` są puste, i tak klika stopę / ring (nie `no_move_marks`). Po kliku czeka aż unit **stanie ON** (re-select / wait / weryfikacja kafelka). `ok` true **tylko** przy `stood_on_city`. `POST /capture` gdy unit ON: jeśli panel nie pokazuje Capture, **najpierw select occupanta** (stopa sprite, nie dach / TRAIN). Blob w UNIT_CROP → klik; `entering city` / `capture_soon` → `not_ready_until_next_turn`. Garnizon żyje → `city_occupied`. `POST /recruit {"city_id":"c0"}` — nameplate → TRAIN blob w panelu.
