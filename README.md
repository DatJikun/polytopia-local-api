...
- `POST /move-to` — niebieskie marki po select `from_id`; cel = **stopa hexu** (~`plate_y` − 0.45 pitch), nie dach i nie sąsiedni niebieski. Klika znaleziony blue / lokalny pierścień pikseli. Gdy unit jest ~1 hex od `city_id` i clustered marki są puste, klika stopę (nie `no_move_marks`). `city_id` ze sticky index nawet gdy last observe nie ma miasta. `stood_on_city` po re-observe; `ok` true tylko wtedy. Negatyw: `no_mark_on_city_tile` / `out_of_range` / `not_stood_on_city` + `path_reason` + `suggested_tile_xy`. `ok` zawsze bool (nigdy `null`). `{x,y}` na kafelku miasta idzie tą samą ścieżką co `city_id`.
