...
4. W-path (Disrof): `POST /attack {"from_id":"uX","city_id":"cY"}` — czerwony mark przy **kafelku miasta** (nie pasek HP giganta), HP z paska unita na kafelku (nie `n`/`w` plate), `hp_dropped` albo jasne `no_actions` / `no_mark` **szybko** (nie timeout 7.5s). `POST /move-to {"from_id":"uZ","city_id":"cY"}` — klika **tylko** niebieski mark ON tile (<0.65 hex); sąsiedni hex (~1.0) to nie miasto. Gdy unit jest w zasięgu chodu w tej turze i kafelek pusty, klika `tile_xy` (budynek, nie plate). `ok` jest true **tylko** przy `stood_on_city` (panel Capture po kliku też się liczy). Sąsiad bez wejścia → `ok:false` / `not_stood_on_city` albo `no_mark_on_city_tile` + `path_reason`. `POST /capture {"city_id":"cY"}` tylko gdy unit ON (nie adjacent) + blob w UNIT_CROP; inaczej `not_standing_on_city` bez kliku; `captured` gdy enemy straci `city_id`. `POST /recruit {"city_id":"c0"}` — nameplate → TRAIN blob w panelu (nie tylko modal na mapie).
...
- `POST /move-to` — niebieskie marki po select `from_id`; cel = środek hexu budynku (~`plate_y` minus ~0.75 hex), **nie** sąsiedni niebieski. W zasięgu chodu klika `tile_xy`. `stood_on_city` po re-observe; `ok` true tylko wtedy (nigdy `ok:true` + `stood_on_city:false` dla city_id). Negatyw: `no_mark_on_city_tile` / `not_stood_on_city` + `suggested_tile_xy` + `path_reason`. `ok` zawsze bool (nigdy `null`). `{x,y}` na kafelku miasta idzie tą samą ścieżką co `city_id`.
